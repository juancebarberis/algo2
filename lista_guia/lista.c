...
/**
 * Ejercicio 6 de guia de TDA
 * */
void *devolver_a_k_posiciones(lista_t *lista, int k)
{
    ...
    while(actual)
    {
        nodo_t *actual_secundario = actual;
        for (int i = 0; i < k; i++)
        {
            if (i + 1 == k && !actual_secundario)
                return actual->dato;
            else
                actual_secundario = actual_secundario->proximo;
        }
        actual = actual->proximo;
    }
    return NULL;
