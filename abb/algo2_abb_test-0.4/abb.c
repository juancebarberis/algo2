 ...
 * un nodo_t* donde se guardará el padre del nodo devuelto (si se devolvió NULL, el último nodo en el camino).
 ...
 * más a la izquierda del hijo derecho del nodo (o sea, el mayor que le sigue a nodo en todo el árbol).
 * */
nodo_t* obtener_reemplazante(nodo_t* nodo, nodo_t** padre_remplazante);
...
{   
    ...
    if(!nodo){ // Si la clave no estaba en el árbol
        ...
        if(!padre) // Si el árbol está vacío
...
{   
    ...
    if(!nodo_a_borrar_previo) 
        nodo_a_borrar = buscar_nodo(arbol, clave, arbol->raiz, &padre); // Si en padre sigue habiendo NULL, el nodo es la raíz
    else nodo_a_borrar = nodo_a_borrar_previo;
    
    if(!nodo_a_borrar) return NULL;

    if(!(nodo_a_borrar->nodo_izq || nodo_a_borrar->nodo_der)) // Si no tiene ningun hijo
    {
        if(!padre) // Si es la raíz 
            arbol->raiz = NULL;
...
void abb_in_order(abb_t *arbol, bool visitar(const char *, void *, void *), void *extra)
{
    ...
    {
        if(actual->nodo_der)
            apilar_nodos(pila, actual->nodo_der);
