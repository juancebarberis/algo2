...
/**
 * Itera el ABB desde la clave inicio (incluida) a la clave fin (incluida). Si alguna de las dos no está, simplemente itera desde/hasta la clave
 * más cercana. Se puede pasar NULL como inicio (en cuyo caso empieza desde la primer clave) y NULL como final (itera hasta la última clave).
 * Usa la función visitar que recibe la clave, el valor y un puntero extra, y devuelve true si se debe seguir iterando, 
 * false en caso contrario.
 * Pre: La estructura ABB fue inicializada.
*/
void abb_por_rangos(abb_t *arbol, bool visitar(const char *, void *, void *), void *extra, const char* ini, const char* fin);
